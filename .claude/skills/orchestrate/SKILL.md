...
**The panel** is the status line: Claude Code re-runs it on every assistant message and pins each line of its output to the bottom of the terminal. So the headings sit in one place and stop scrolling away, which is what Ita asked for (coo#87): *"i just want to have the running, ran, planned in a fixed format that doesnt go anywhere instead of the text written at the end of every msg."* There is nothing to set up per pass — `plan` brings the panel up, and a session that has never run a pass shows no headings at all. **A finished pass stays on it**: after the final report the Done rows, the orchestrator's row and the total are still pinned, until Ita dismisses them (Ita, 2026-09-22, coo#137: *"can you add a task so that it stays? like even when youre done coz now id wanna see it also at the bottom and not as part of your prompt"*). **Done accumulates for the conversation**: the next pass's `plan` puts its own rows above the last pass's Done rows rather than replacing them, and only `clear`, or `/clear` starting a new conversation, empties Done (Ita, 2026-09-22, coo#142: *"also the done tasks should still be in the done list until the session is cleared"*). `/home/ita/coo/tools/orchestrate-status clear` is the dismiss — his to run from the prompt, or yours when he asks; the skill never runs it.

**It is one table across two panels.** Claude Code's own subagent panel draws a row for every live worker, and since coo#104 each of those rows *is* its pass row; the pinned panel carries the rest — the orchestrator's row, Planned, Done and the total — and leaves off every row the subagent panel is drawing, so nothing is on screen twice (Ita, 2026-09-22, coo#135: *"if the subagents thing insists on showing only running tasks, we can just show the planned and done under it?"*). The subagent panel records which rows it drew on each tick, beside the pass file; the pinned panel reads that record and hides those rows and no others. So a Running row whose worker has no line up there — not spawned yet, evicted 30 seconds after it ended while you have not landed it, spawned by another session — stays on the pinned panel, and a record more than 15 seconds old is ignored, so a subagent panel that has stopped drawing hands every row back. A stage whose rows are all up there is left out rather than printed as `—`. `/home/ita/coo/tools/orchestrate-status show` prints the table whole and plain on stdout, never split.

**The bottom of the table rides under the last worker.** Claude Code draws the status line *above* the prompt's hint line and its subagent panel *below* it, in fixed code with no setting to swap them, so two panels read as one table upside down — Planned and Done over Running (Ita, 2026-09-22, coo#138: *"its showing the subagents at the bottom and the planned and done at the top"*). So while any worker has a row down there, Planned, Done and the total are packed as extra lines into the **last** of those rows (the one started latest — the panel orders by start time), and the pinned panel keeps only any Running row with no worker line. Top to bottom it then reads Running, Planned, Done, total. Those extra lines belong to that worker's row, so clicking one opens that worker, and they are pinned to the grey Claude Code gives an unselected row, so selecting or hovering it changes nothing but the `❯` pointer — but while that worker is open (a click on any of its lines opens it) its whole row is bold, which a terminal draws brighter, the tail with it, and nothing a row's content carries can undo that (coo#143). A decorated row is only our text, so it loses Claude Code's own per-second elapsed and token count, and its ELAPSED steps every 5 seconds, when the hook is re-run (coo#156); with no worker listed, the record goes stale within 15 seconds and the whole table is back on the pinned panel. The subagent panel's own `● main` row is drawn by Claude Code itself — a fixed word, no count, not in what the hook is handed — so it cannot read "orchestrator". **The orchestrator's count is on the model line** instead: the wrapper appends it to the last line it prints from Giverny, `Opus 5.5  ·  5h 55%  ·  wk 60%  ·  session 115.4k`, at render time and without ever writing Giverny's config (Ita, 2026-09-22, coo#141: *"you cant put the token count on the main row? like thats the only thing we need from these entire two rows..."*, then *"dont call it orch... maybe session"*). A usage limit the pass is stalled on goes on the end with it, `session 115.4k  ·  5h limit → 15:20`; with nothing printed above, the count is a line of its own. So the pinned panel has no `Session` row. `show` still prints the orchestrator as the table's top row, having no model line to lean on.

**The panel is the only view — nothing is pasted into a message, the final report included.** The lists went on every message until coo#97 (Ita: *"you dont need to write the suffix coz the under table thing seems ot be working!"*), then on the final report alone, and since coo#137 on nothing: the panel re-renders on every message and on a timer, it stays after the pass, and a block pasted under a report only says it twice. It looks like this:

```
Opus 5.5  ·  5h 55%  ·  wk 60%  ·  session 206.3k
Running  yajna#74 CLEANUP: retire queue.json             14:05        ~6m  Editing tools/board             124.8k
Running  yajna#76 FEATURE: one change, second issue          "          "  "                                    "
Planned  yajna#75 RUN: re-measure board cost                          ~26m
Done     yajna#73 FEATURE: lane carries its own backlog   17:32             Review — ita                    213.3k
Done     yajna#71 BUG: board loses the middle page      1:02:10             Done                            230.3k
                                                                                                    total:  774.7k
```

It is a table — the orchestrator's own row at the top (in `show`; on the panel it is the model line's `session` count), then Running, then Planned, then Done, the stage written on every row, and the total on the last — in the columns STAGE, TASK, ELAPSED, ETA, NOW and TOKENS, every one but TASK a fixed width and TASK taking the rest of the terminal, so nothing jumps between renders (Ita, 2026-09-17, coo#90: *"running then planned then ran … stage, task, eta in fixed length columns"*). **There is no header row** — the columns stand on alignment (Ita, 2026-09-23, coo#149: *"no need for the headers of stage/tst/elapsed/etc"*). The count is the last column, so the eye scans a straight edge down the right of the table (Ita, 2026-09-19, coo#117: *"the tokens should be the most right column"*). Claude Code runs the status line with `COLUMNS` and `LINES` set to the terminal's size, so the panel sizes itself to the window; `/home/ita/coo/tools/orchestrate-status width <cols>` is only the fallback for a status line run some other way, and unset it lays out for 120. Every render comes from the same file, so the pinned panel, the subagent panel and `show` cannot disagree, and what the renderer guarantees you no longer have to:

- **Every line names the task by id *and* title.** A title longer than the column is cut with an ellipsis; the id never is.
- **Three clock columns, each one kind of thing.** **ELAPSED** is how long a row has been going, or took; **ETA** is how much longer until it is finished; **NOW** is what a Running row's worker is doing this minute, or where a Done row landed (Ita, 2026-09-23, coo#149/coo#150: *"add an elapsed time also next to the eta.. so the done ones just show the elapsed time"*, and *"add another col for whatever the agent is doing now in the task? … it can actually be the thing in the done/review col and the elapsed can go to its left"*). So a Running row fills all three, a Done row has ELAPSED and NOW and no ETA, and a Planned row has only its ETA.
- **Durations read `~5m`, `~1h3m`, `~1d3h12m`** (Ita, 2026-09-23, coo#145: *"like ~5m or ~1h3m or ~1d3h12m with the tildas"*), units that are zero left out — `~1h`, not `~1h0m`; the tilde says it is an estimate. A row past its estimate keeps a leading minus, `-~12m`, a countdown through zero. **ELAPSED is a stopwatch instead** — `0:42`, `14:05`, `1:03:07`, only the digits it needs (Ita, 2026-09-23: *"elapsed should be hh:mm:ss but only what the digits are necessary ofc"*) — measured, so no tilde, and `start`/`land` stamp to the second for it. Both columns are right-aligned. The finish clock, the `since 14:20` and the `lane 2 after #74` note that once shared the ETA column are long gone: the lanes are still chained exactly as they were, and which lane a task is in is §2's business and the report's.
- **NOW on a Running row is the worker's own activity.** On Claude Code's subagent panel it is the line Claude Code itself would show for that worker — its progress summary, handed to the hook as `tasks[].label` — and everywhere else, or before the CLI has a summary, it is the last tool call or line of text in the worker's transcript (`Reading board.html`, a Bash call's description). Cut to its column; the id and the numbers never move.
- **Clocks survive midnight.** `start` and `land` write the full date and time (`2026-09-23T00:12:07`), and a bare `HH:MM` in a file written before that is read as the latest such time at or before now, so a row started at 23:48 reads `14:00` gone at 00:02, not a day (coo#144).
- **Done** is every task finished so far this conversation — this pass and every pass before it since the last `clear` (coo#142) — the span it took, and where it landed — **newest first**, so the last thing to land is the top Done row (Ita, 2026-09-19, coo#117: *"the last done should be at the top"*). A Review line names its reviewer; a Blocked line names its blocker, in NOW. **The stage means this pass is done with the task, not that the board says `Done`** — a `Done` row can read `Review — ita` or `Blocked on inbar#40` in NOW, because that is where it landed and the stage is only that the pass let go of it. It was called `Ran` until 2026-09-19 (Ita, coo#110: *"ran in green and it should be called done"*); the plan file still writes the state as `ran`, which nothing but the renderer reads.
- **Running** counts itself down. `~20m` is `start + eta − now`, recomputed every time the panel re-renders, so it is true between your messages as well as in them — and a worker past its estimate goes to `-~12m` instead of sitting at "20m left" forever. **A usage limit brings its own wait with it:** once one is actually hit, every Running and Planned row grows by the time until that limit resets — `~20m` reads `~1h20m` the moment a five-hour limit goes at an hour to the reset, shrinks as the reset comes round and is back to `~20m` when it lands — because nothing is running in between, and a panel counting two workers killed by the session limit down to `-02:30` for two hours is what Ita saw on 2026-09-22 (coo#130: *"the eta timer needs to adjust to the token limits"*). Until a limit is hit it changes nothing, and the orchestrator's count on the model line names the one that is out.
...
- Times are measured, never guessed: `start` and `land` stamp the clock themselves. ELAPSED was timed, to the second; ETA is the `--eta` you gave, counted down, and its `~` says so (coo#145). Once the first worker lands, re-run `plan` to re-estimate the rest against what it actually took.
...
Per item: what it was, what its worker did, how it was verified, where it landed (Done / Review-and-who / Blocked-and-why), and what it left behind. Then the queue's new state, anything you skipped with the reason, and the priorities you changed. No footer and no `show`: the panel under the prompt already carries every row with its actual timing, and it stays there after this message (§4). Do not `clear` it — the next pass's `plan` adds to it, and dismissing it is Ita's call.
